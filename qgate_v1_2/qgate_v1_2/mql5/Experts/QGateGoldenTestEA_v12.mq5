#property strict
#property description "QGate v1.2 Golden Test EA (no trading). Put CSVs in MQL5/Files."

#include <QGateV12.mqh>
...
input string PROFILE = "m15_h1"; // "m5_m30" or "m15_h1"
...
struct BarRow { datetime t; double o,h,l,c; };

static bool ReadBarsCSV(const string filename, BarRow &rows[]){
   int fh = FileOpen(filename, FILE_READ|FILE_CSV|FILE_ANSI);
   if(fh == INVALID_HANDLE){ Print("Failed open ", filename, " err=", GetLastError()); return false; }

   if(!FileIsEnding(fh)){
      FileReadString(fh); FileReadString(fh); FileReadString(fh); FileReadString(fh); FileReadString(fh);
   }
   ...
   for(int i=1;i<n;i++){
      BarRow key=tmp[i];
      int j=i-1;
      while(j>=0 && tmp[j].t > key.t){ tmp[j+1]=tmp[j]; j--; }
      tmp[j+1]=key;
   }
...
struct GoldenRow {
   datetime t_ltf;
   double q_ltf; string state_ltf; string band_ltf; int allow_ltf;
   datetime t_htf_map;
   double q_htf; string state_htf; string band_htf; int allow_htf;
   int allow_combined;
};
...
static bool ReadGoldenOut(const string filename, GoldenRow &rows[]){
   ...
   for(int i=1;i<n;i++){
      GoldenRow key=tmp[i];
      int j=i-1;
      while(j>=0 && tmp[j].t_ltf > key.t_ltf){ tmp[j+1]=tmp[j]; j--; }
      tmp[j+1]=key;
   }
   ArrayResize(rows,n);
   for(int i=0;i<n;i++) rows[i]=tmp[i];
...
static int _StateFromStr(const string s){ return (s=="PASS" ? (int)GS_PASS : (int)GS_FAIL); }
...
static void FillProfileCfgs(const string profile, QGateV12Cfg &cfgL, QGateV12Cfg &cfgH){
   cfgL.atrPeriod=14; cfgL.atrBaselineLen=50; cfgL.atrRatioRef=1.25;
   cfgL.erLookback=20; cfgL.erRef=0.35;
   cfgL.tratrRef=1.0;

   cfgL.adx.enabled=true;
   cfgL.adx.period=14;
   cfgL.adx.ref=25.0;
   ...
   if(profile == "m5_m30"){
      cfgL.cls.hysteresis=true; cfgL.cls.q_pass=0.70; cfgL.cls.q_fail=0.62;
      cfgL.wAtr=0.28; cfgL.wEr=0.28; cfgL.wTr=0.14; cfgL.wAdx=0.30;

      cfgH = cfgL;
      cfgH.cls.hysteresis=true; cfgH.cls.q_pass=0.72; cfgH.cls.q_fail=0.68;
      cfgH.wAtr=0.30; cfgH.wEr=0.30; cfgH.wTr=0.15; cfgH.wAdx=0.25;
   } else {
      cfgL.cls.hysteresis=true; cfgL.cls.q_pass=0.72; cfgL.cls.q_fail=0.66;
      cfgL.wAtr=0.28; cfgL.wEr=0.28; cfgL.wTr=0.14; cfgL.wAdx=0.30;

      cfgH = cfgL;
      cfgH.cls.hysteresis=true; cfgH.cls.q_pass=0.75; cfgH.cls.q_fail=0.70;
      cfgH.wAtr=0.30; cfgH.wEr=0.30; cfgH.wTr=0.15; cfgH.wAdx=0.25;
   }
}
...
int OnInit(){
   ...
   QGateV12Cfg cfgL, cfgH;
   FillProfileCfgs(PROFILE, cfgL, cfgH);

   QGateV12Out outL[], outH[];
   ComputeQGateV12_All(tL, oL, hL, lL, cL, cfgL, outL);
   ComputeQGateV12_All(tH, oH, hH, lH, cH, cfgH, outH);
   ...
   Print("==== QGate v1.2 Golden Test Summary (profile=", PROFILE, ") ====");
